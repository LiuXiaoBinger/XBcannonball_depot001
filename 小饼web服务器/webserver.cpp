...
void WebServer::start() {
    ...
    //pthread_t tid;
    while(1) {
        event_num = _epoller->wait(TIMESHOT);
        for(int i = 0; i < event_num; i++) {
            ...
            //     }
                ...
                // if(timeout) {
                //     auto i = std::thread(&WebServer::handleTimer,this);
                //     i.detach();
...
void WebServer::http_request(const char* request,int cliScok) {
    ...
    //strcmp两个字符串⾃左向右逐个字符相⽐（按ASCII值⼤⼩相⽐较），
    //直到出现不同的字符或遇'\0'为⽌。如："A"<"B" "a">"A"
//     当s1<s2时，返回值<0
// 　　当s1=s2时，返回值=0
// 　　当s1>s2时，返回值>0
    if(strcmp(path, "/") == 0) {    
...
//设置心跳模块
void WebServer::setKeepAlive(int cliSock) {
    int iKeepAlive = 1;
    setsockopt(cliSock, SOL_SOCKET, SO_KEEPALIVE, (void *)&iKeepAlive, sizeof(iKeepAlive));
}
